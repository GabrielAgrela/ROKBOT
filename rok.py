...
def chooseAnswer(question):
	...
	for i in range(249):
		if (question_list[-3] in questions[i]):
			#print (answers[i],"\n")
			searchOption(answers[i])

#receives answers from chooseAnswer() and checks which one is in the display's options
def searchOption(answer):
	if (answer in getTextFromImage(.25, .54, .4, .49)):
		tap(.45,.3)
	elif (answer in getTextFromImage(.60, .89, .4, .49)):
		tap(.45,.7)
	elif (answer in getTextFromImage(.25, .54, .54, .62)):
		tap(.58,.3)
	elif (answer in getTextFromImage(.60, .89, .54, .62)):
		tap(.6,.7)
	else:
